...
QUnit.module('Тестируем функцию roman', function () {
  ...
  QUnit.test('roman правильно определяет, что было передано на вход', function (
    assert
  ) {
    assert.strictEqual(roman('1904'), 'MCMIV');
    assert.strictEqual(roman('1990'), 'MCMXC');
  ...
  QUnit.test('roman правильно определяет, что значение некорректно', function (
    assert
  ) {
    ...
    assert.throws(() => roman('wdde'), Error('некорректное число'));
