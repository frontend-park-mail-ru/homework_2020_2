...
QUnit.module('Тестируем функцию format', function () {
	...
	QUnit.test('format работает правильно, если в качестве количества колонок подается не целое число', function (assert) {
		const input = [0, 1];
	...
	QUnit.test('format работает правильно, если подается пустой массив', function (assert) {
		const input = [];
	...
	QUnit.test('format работает правильно, если в массиве есть нецелые числа или другие типы данных', function (assert) {
		const input1 = [ -1.5, 4];
		const input2 = [ 't', 4];
		const input3 = [ {}, {} ];
		...
		assert.throws(() => format(input3, 2), expected);
	});
